//! Local locked-alternate-program Lift15 acquisition and replay, without campaign authority.
...
use crate::{
    ...
    recursive::{LocalAlternateProgramInput, RECURSIVE_ORACLE_MAX_BYTES, admit_local_alternate_program},
};

const INPUT_ROLES: [&str; 4] = ["canonical-statement", "case9-recursive-oracle", "case9-assumption-raw-seal", "case9-final-raw-seal"];
const PAYLOAD_NAMES: [&str; 3] = ["alternate-program.receipt-oracle.bincode", "alternate-program.raw-seal.bin", "alternate-program-statement.bin"];
const MANIFEST: &str = "alternate-program-checkpoint.json";
...
impl Checkpoint {
    fn encode(&self) -> Result<Vec<u8>> {
        ...
        ensure!(bytes.len() <= MANIFEST_MAX_BYTES, "local alternate manifest byte bound");
        Ok(bytes)
    }

    fn decode(bytes: &[u8]) -> Result<Self> {
        ensure!(!bytes.is_empty() && bytes.len() <= MANIFEST_MAX_BYTES, "local alternate manifest byte bound");
        let value: Self = serde_json::from_slice(bytes).context("local alternate manifest typed JSON")?;
        ensure!(value.encode()? == bytes, "local alternate manifest is not canonical");
        Ok(value)
    }
}
...
impl Inputs {
    fn read(paths: [&Path; 4]) -> Result<Self> {
        ...
        let files: [Snapshot; 4] = files.try_into().map_err(|_| anyhow::anyhow!("local alternate input cardinality"))?;
        ensure!(files.iter().map(Snapshot::node).collect::<BTreeSet<_>>().len() == 4,
            "local alternate input identities are duplicated");
        ensure!(files[2].bytes().len() == PROOF_BYTES && files[3].bytes().len() == PROOF_BYTES,
            "local alternate Case9 raw seal byte length");
        Ok(Self { files })
    }

    fn admit(&self, locked: &LockedGuestPairV1) -> Result<LocalAlternateProgramInput> {
        admit_local_alternate_program(locked, self.files[0].bytes(), self.files[1].bytes(),
            self.files[2].bytes(), self.files[3].bytes())
    }

    fn verify(&self) -> Result<()> {
        for file in &self.files { file.verify().context("local alternate original input conservation")?; }
        Ok(())
    }
}

fn expected_checkpoint(inputs: &Inputs, locked: &LockedGuestPairV1,
                       payloads: &[Vec<u8>; 3], outer: &[u8]) -> Checkpoint {
    let guest = |(elf, id): (&[u8], risc0_zkvm::Digest)| GuestPin { elf: Pin::of(elf), image_id: hex::encode(id.as_bytes()) };
    Checkpoint {
        schema: "eip0045.local-ancestry-alternate-program.v1".to_owned(), version: 1,
        role: "locked-alternate-program".to_owned(), terminal: "stock-lift15".to_owned(),
...
fn replay_checkpoint(inputs: &Inputs, locked: &LockedGuestPairV1, admitted: &LocalAlternateProgramInput,
                     payloads: &[Vec<u8>; 3]) -> Result<Checkpoint> {
    ensure!(payloads[2] == admitted.statement(), "local alternate derived statement mismatch");
    let receipt = admitted.replay(&payloads[0], &payloads[1])?;
    Ok(expected_checkpoint(inputs, locked, payloads, &borsh::to_vec(&receipt)?))
}

/// Prove a fixed locked-alternate-program Lift15 from four retained Case9 inputs.
///
/// # Errors
...
pub fn generate(paths: [&Path; 4], output_root: &Path) -> Result<()> {
    ...
    let result = (|| {
        let original = admitted.prove(&LocalProver::new("local-ancestry-alternate-program"))?;
        let (oracle, raw, outer) = admitted.checkpoint_parts(&original)?;
...
/// Reopen exactly four files and replay against original Case9 inputs and locked guests.
///
/// # Errors
...
fn payload_snapshots(root: &Directory) -> Result<[Snapshot; 3]> {
    ...
    files.try_into().map_err(|_| anyhow::anyhow!("local alternate payload cardinality"))
}
...
fn publish(root: &Directory, payloads: &[Vec<u8>; 3], expected: &Checkpoint,
    ...
    ensure!(replay(&payload_bytes(&files))? == *expected, "local alternate rereplay identity mismatch");
...
fn verify_published(root: &Directory, mut replay: impl FnMut(&[Vec<u8>; 3]) -> Result<Checkpoint>,
    ...
    ensure!(nodes.insert(manifest.node()) && nodes.len() == 4, "local alternate checkpoint identities are duplicated");
    ensure!(replay(&payload_bytes(&files))? == expected, "local alternate manifest identity mismatch");
    for file in &files { file.verify()?; }
    manifest.verify()?;
    require_names(root, &names)?;
    root.verify()?;
...
mod tests {
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            let root = std::env::temp_dir().join(format!("local-alternate-program-tests-{}-{}-{}",
    ...
    #[test]
    fn alternate_manifest_requires_exact_typed_canonical_bytes() {
    ...
    #[cfg(unix)]
    #[test]
    fn alternate_input_join_rejects_alias_length_and_replacement_then_restores() {
        ...
        assert_eq!(Inputs::read(refs).err().unwrap().to_string(), "local alternate Case9 raw seal byte length");
    ...
    #[cfg(unix)]
    #[test]
    fn alternate_publication_reopens_exact_payloads_and_conserves_last() {
    ...
    #[cfg(unix)]
    #[test]
    fn alternate_publication_retains_first_payload_identity_across_conservation() {
    ...
    #[cfg(unix)]
    #[test]
    fn alternate_manifest_bindings_are_rederived_not_self_authorized() {
        let fixture = Fixture::new(); let root = create_fresh_directory(&fixture.0.join("checkpoint")).unwrap();
        let payloads = [vec![1], vec![2], vec![3]]; let expected = synthetic_manifest(&payloads);
        ...
        for fault in ["schema", "version", "role", "terminal", "input-role", "input-pin", "consumer-elf",
            ...
            match fault {
                ...
                "role" => changed.role.push('x'), "terminal" => changed.terminal = "stock-lift16".to_owned(),
            ...
            assert_eq!(verify_published(&root, |_| Ok(expected.clone()), || Ok(())).unwrap_err().to_string(),
                "local alternate manifest identity mismatch", "{fault}");
    ...
    #[cfg(unix)]
    #[test]
    fn alternate_replay_and_final_conservation_failures_preserve_partial_output() {
        for fault in ["replay", "late-replay", "conservation"] {
            ...
            let error = publish(&root, &payloads, &expected, |_| {
                count += 1;
                ensure!(fault != "replay" && !(fault == "late-replay" && count == 2), "synthetic alternate replay denied");
            ...
            assert_eq!(error.to_string(), if fault == "conservation" { "local file identity drift" }
                else { "synthetic alternate replay denied" });
    ...
    #[cfg(unix)]
    #[test]
    fn alternate_reread_closes_payload_inventory_alias_and_replacement_faults() {
        ...
        for fault in ["extra", "missing", "hardlink", "symlink", "bytes", "replaced-after-replay", "manifest-race"] {
            ...
            let error = verify_published(&root, |actual| {
                ensure!(actual == &payloads, "synthetic alternate payload drift");
            ...
            let expected_error = match fault {
                ...
                "hardlink" => Some("local file must have one link"), "bytes" => Some("synthetic alternate payload drift"),
    ...
    fn genuine_paths() -> Result<[PathBuf; 4]> {
        ["EIP0045_ALT_PROGRAM_CANONICAL_STATEMENT", "EIP0045_ALT_PROGRAM_CASE9_ORACLE",
         "EIP0045_ALT_PROGRAM_CASE9_ASSUMPTION_RAW", "EIP0045_ALT_PROGRAM_CASE9_FINAL_RAW"]
    ...
    fn genuine_alternate_admission_rejects_each_original_input_fault() -> Result<()> {
        ...
        for index in 0..4 {
            ...
            let error = admit_local_alternate_program(&locked, &bytes[0], &bytes[1], &bytes[2], &bytes[3])
            ...
            let message = match index {
                ...
                3 => "local alternate Case9 final Resolve raw seal mismatch", _ => unreachable!(),
            };
    ...
    #[cfg(unix)]
    #[test]
    #[ignore = "requires original Case9 inputs and separately proved alternate-program checkpoint; never proves"]
    fn genuine_alternate_checkpoint_replay_and_isolated_proof_faults() -> Result<()> {
        let paths = genuine_paths()?; let refs = std::array::from_fn(|i| paths[i].as_path());
        let root = PathBuf::from(std::env::var_os("EIP0045_ALT_PROGRAM_CHECKPOINT").context("missing alternate checkpoint")?);
        verify(refs, &root)?;
        let inputs = Inputs::read(refs)?; let locked = embedded_negative_ancestry_guest_pair()?;
        ...
        assert_eq!(admitted.checkpoint_parts(&original)?.2, borsh::to_vec(&original)?);
        for fault in ["terminal", "parameters", "claim", "raw", "oracle", "seal-rebound", "trailing", "outer"] {
            ...
            match fault {
                "terminal" => inner.control_id = crate::expected_normal_lift_control_id(16)?,
                "parameters" => { let mut bytes = [0_u8; 32]; bytes.copy_from_slice(inner.verifier_parameters.as_bytes());
                    bytes[0] ^= 1; inner.verifier_parameters = risc0_zkvm::Digest::from_bytes(bytes); },
                "claim" => inner.claim = risc0_zkvm::MaybePruned::Pruned(risc0_zkvm::Digest::ZERO),
                "raw" => raw[0] ^= 1, "oracle" => inner.seal[0] ^= 1,
                "seal-rebound" => { inner.seal[0] ^= 1; raw = inner.get_seal_bytes(); },
                "trailing" | "outer" => (), _ => unreachable!(),
            }
            if fault == "outer" {
                let mut changed = original.clone();
                let mut bytes = [0_u8; 32]; bytes.copy_from_slice(changed.metadata.verifier_parameters.as_bytes()); bytes[0] ^= 1;
                changed.metadata.verifier_parameters = risc0_zkvm::Digest::from_bytes(bytes);
                use risc0_zkvm::sha::Digestible as _;
                assert_eq!(original.claim()?.digest(), changed.claim()?.digest());
                assert_ne!(borsh::to_vec(&original)?, borsh::to_vec(&changed)?);
                assert!(admitted.checkpoint_parts(&changed).is_err());
            } else {
                let mut oracle = crate::receipt_oracle_wire::encode_succinct_receipt_oracle(&inner)?;
                if fault == "trailing" { oracle.push(0); }
                let error = admitted.replay(&oracle, &raw).err().context("changed proof was accepted")?;
                if matches!(fault, "raw" | "oracle") {
                    assert_eq!(error.to_string(), "local alternate raw seal mismatch");
                } else if fault != "trailing" {
                    assert_eq!(error.to_string(), "supplied assumption receipt failed exact Lift-15 validation",
                        "isolated proof fault {fault}: {error:#}");
                }
            }
            admitted.replay(&payloads[0], &payloads[1])?;
        }
        let mut changed = payloads.clone(); changed[2][0] ^= 1;
        assert_eq!(replay_checkpoint(&inputs, &locked, &admitted, &changed).unwrap_err().to_string(),
            "local alternate derived statement mismatch");
