...
//! Pure codec for the canonical B2 verifier-data artifact.
//!
//! This module performs no filesystem or source-extraction work. It accepts
//! bytes already supplied by the caller, enforces the complete fixed grammar,
//! and authenticates the one frozen 65,119-byte artifact.
...
/// Exact upstream RISC Zero commit from which the frozen data was derived.
pub const RISC0_COMMIT: &str = "8eb06ab020a92dc5b63ba6dd0836d432aba6d890";
/// Exact canonical B2 artifact byte length.
...
/// Complete decoded B2 verifier-data artifact.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ConstantsArtifact {
    /// Pinned upstream RISC Zero commit.
    pub upstream_commit: String,
    /// `BabyBear` field parameters and roots.
    pub baby_bear: BabyBearData,
    /// `Poseidon2` parameters and constants.
    pub poseidon2: Poseidon2Data,
    /// STARK and FRI scalar parameters.
    pub stark: StarkData,
    /// Fixed proof-system and circuit protocol strings.
    pub protocol_info: ProtocolInfoData,
    /// Recursion-circuit tap table.
    pub taps: TapDataSet,
    /// Recursion-circuit constraint operations.
    pub poly_ext: PolyExtData,
}

/// `BabyBear` field data encoded by B2.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct BabyBearData {
    ...
    /// Extension polynomial beta.
    pub extension_beta: u32,
    /// Largest supported root-of-unity exponent.
    pub max_root_po2: u8,
    /// Reverse root-of-unity table.
    pub reverse_roots: Vec<u32>,
    /// Extension coefficient remapping order.
    pub extension_coefficient_remap: [u8; 4],
    /// Extension challenge scale.
    pub extension_challenge_scale: u8,
}

/// `Poseidon2` permutation data encoded by B2.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct Poseidon2Data {
    ...
    pub output_cells: u8,
    /// Half-full-round count.
    pub rounds_half_full: u8,
    /// Partial-round count.
    pub rounds_partial: u8,
    /// S-box degree.
    pub sbox_degree: u8,
    /// Sparse canonical round constants.
    pub round_constants: Vec<u32>,
    /// Internal matrix diagonal constants.
    pub internal_diagonal: Vec<u32>,
}

/// STARK and FRI scalar data encoded by B2.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct StarkData {
    ...
    /// FRI fold factor.
    pub fri_fold: u8,
    /// Base-two logarithm of the FRI fold factor.
    pub fri_fold_po2: u8,
    /// Minimum FRI degree.
    pub fri_min_degree: u16,
    /// Circuit output width.
    pub output_size: u8,
    /// Circuit mix width.
    pub mix_size: u8,
    /// Circuit check width.
...
/// Fixed protocol-info byte strings encoded by B2.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ProtocolInfoData {
    /// Proof-system protocol string.
    pub proof_system: String,
    /// Recursion-circuit protocol string.
    pub circuit: String,
}

/// Complete recursion tap data encoded by B2.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct TapDataSet {
    /// Register counts for the three tap groups.
    pub group_sizes: [u8; 3],
    /// Number of tap combinations.
    pub combos_count: u8,
    /// Total number of combination back offsets.
    pub total_combo_backs: u8,
    /// Canonically ordered tap records.
    pub records: Vec<Tap>,
}

/// One five-byte recursion tap record.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize)]
pub struct Tap {
    /// Register group.
    pub group: u8,
    /// Register offset within the group.
    pub offset: u8,
    /// Back-reference distance.
    pub back: u8,
    /// Combination index.
    pub combo: u8,
    /// Width of the register's tap run.
    pub skip: u8,
}

/// Recursion constraint-program metadata and `PolyExt` operations.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct PolyExtData {
    /// Number of field-valued variables.
    pub field_variables: u16,
    /// Number of extension-mix variables.
    pub mix_variables: u16,
    /// Final returned mix-variable index.
    pub ret_mix_variable: u16,
    /// Operation histogram derived from the instruction stream.
    pub histogram: BTreeMap<String, usize>,
    /// Canonical recursion constraint instruction stream.
    pub operations: Vec<PolyExtOp>,
}

/// One `PolyExt` recursion constraint instruction.
#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
#[serde(tag = "op", content = "args")]
pub enum PolyExtOp {
    /// Introduce one base-field constant.
    Const(u32),
    /// Introduce one extension-field constant.
    ConstExt([u32; 4]),
    /// Read one tap.
    Get(u16),
    /// Read one global value.
    GetGlobal([u16; 2]),
    /// Add two earlier field variables.
    Add([u16; 2]),
    /// Subtract two earlier field variables.
    Sub([u16; 2]),
    /// Multiply two earlier field variables.
    Mul([u16; 2]),
    /// Start a true mix chain.
    True,
    /// Conjoin an earlier field equality-to-zero with a mix chain.
    AndEqz([u16; 2]),
    /// Conditionally conjoin two earlier mix values.
    AndCond([u16; 3]),
}

/// Encode the unique canonical B2 representation.
///
/// # Errors
///
/// Returns an error if any field, table, reference, or census differs from the
/// frozen B2 grammar.
...
/// Decode exactly 65,119 B2 bytes and enforce the complete fixed grammar.
///
/// # Errors
///
/// Returns the first length, scalar, table, opcode, reference, or census
/// violation.
...
mod tests {
    ...
    #[test]
    fn mutations_across_every_serialized_region_lose_frozen_authentication() {
        let taps_offset = HEADER_BYTES
            + ROOT_COUNT * 4
            + POSEIDON_ROUND_CONSTANTS * 4
            + POSEIDON_DIAGONAL_CONSTANTS * 4;
        let operations_offset = taps_offset + TAP_COUNT * TAP_BYTES;
        let mutation_offsets = [
            0,
            HEADER_BYTES - 1,
            HEADER_BYTES,
            HEADER_BYTES + ROOT_COUNT * 4 - 1,
            HEADER_BYTES + ROOT_COUNT * 4,
            HEADER_BYTES + ROOT_COUNT * 4 + POSEIDON_ROUND_CONSTANTS * 4 - 1,
            taps_offset - 1,
            taps_offset,
            operations_offset - 1,
            operations_offset,
        ...
        for index in mutation_offsets {
            ...
            changed[index] ^= 1;
            assert!(
                verify_canonical(&changed).is_err(),
                "mutation at byte {index} retained frozen authentication"
            );
        }
    }

    #[test]
    fn decoded_field_mutations_are_rejected_by_the_encoder() {
        let canonical = verify_canonical(FROZEN_B2).unwrap();

        let mut changed = canonical.clone();
        changed.baby_bear.modulus -= 1;
        assert!(encode(&changed).is_err());

        let mut changed = canonical.clone();
        changed.poseidon2.round_constants[0] = changed.baby_bear.modulus;
        assert!(encode(&changed).is_err());

        let mut changed = canonical.clone();
        changed.taps.records[0].skip = 0;
        assert!(encode(&changed).is_err());

        let mut changed = canonical;
        changed.poly_ext.operations.pop();
        assert!(encode(&changed).is_err());
    }
}
